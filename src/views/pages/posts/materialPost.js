...
const MaterialPost = () => {
    ...
    const fetchPosts = async () => {
        return await axios.get('http://127.0.0.1:5000/admin/materialPosts');
    };

    const deletePost = async (id) => {
        await axios
            .delete(`http://127.0.0.1:5000/admin/materialPosts/${id}`)
            ...
            {data.map((item, idx) => (
                ...
                >
                    ...
                    <div style={{ marginTop: '15px', display: 'flex', flexDirection: 'column', gap: '5px', alignItems: 'flex-start' }}>
                        <span>
                            posted by: {item.workerId?.nom} {item.workerId?.prenom}
                        </span>
